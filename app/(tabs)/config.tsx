...
export default function ConfigScreen() {
  ...
  const dataModeLabel = useMemo(() => {
    ...
    return dataMode === 'wifi-only'
      ? 'Apenas Wi-Fi (recomendado)'
      : 'Sempre (Wi-Fi e dados)';
  }, [dataMode, isLoading]);
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View style={styles.headerRow}>
            <Image
              source={require('../../assets/images/mascote-vovo.png')}
              style={styles.avatar}
            />
            <ThemedText style={styles.title}>Configurações do Vovô</ThemedText>
          </View>
          <ThemedText style={styles.subtitle}>
            Controle de conexão, privacidade e dados.
          </ThemedText>
        </View>
        ...
        <GlassCard>
          ...
          <ThemedText style={styles.note}>
            Quando ativado, o app só baixa atualizações no Wi-Fi.
          </ThemedText>
          ...
          <Pressable
            disabled={isLoading}
            onPress={() =>
              setDataMode(dataMode === 'wifi-only' ? 'always' : 'wifi-only')
            }
        ...
        <GlassCard>
          ...
          <ThemedText style={styles.note}>
            Aviso quando detectar um novo resultado.
          </ThemedText>

          <View style={styles.row}>
            <ThemedText style={styles.rowLabel}>Novo resultado</ThemedText>
            <ThemedText style={styles.rowValue}>
              {newResultAlertsEnabled ? 'Ativado' : 'Desativado'}
            </ThemedText>
          </View>
        ...
        <GlassCard>
          ...
          <ThemedText style={styles.disclaimer}>
            Ferramenta independente, sem vínculo com a Caixa.
          </ThemedText>
        </GlassCard>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  header: { marginTop: 8, marginBottom: 8, gap: 6 },
  headerRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  avatar: { width: 36, height: 36, borderRadius: 18 },
  title: { fontSize: 30, fontWeight: '900', color: '#fff' },
  subtitle: { fontSize: 16, color: 'rgba(255,255,255,0.78)' },
  sectionTitle: { fontSize: 18, fontWeight: '900', color: '#fff', marginBottom: 10 },
  note: { fontSize: 14, lineHeight: 20, color: 'rgba(255,255,255,0.78)', marginBottom: 10 },
  row: { flexDirection: 'row', justifyContent: 'space-between', gap: 12 },
  rowLabel: { fontSize: 15, fontWeight: '800', color: 'rgba(255,255,255,0.8)' },
  rowValue: { fontSize: 15, fontWeight: '900', color: '#fff', textAlign: 'right', flex: 1 },
  primaryButton: {
    minHeight: 48,
    borderRadius: 14,
    alignItems: 'center',
  ...
  primaryButtonText: { fontSize: 16, fontWeight: '900', color: '#fff' },
  secondaryButton: {
    minHeight: 48,
    borderRadius: 14,
    alignItems: 'center',
  ...
  secondaryButtonText: { fontSize: 16, fontWeight: '900', color: '#fff' },
  hint: { fontSize: 13, lineHeight: 18, color: 'rgba(255,255,255,0.7)' },
  ...
  pillText: { fontSize: 15, fontWeight: '900', color: 'rgba(255,255,255,0.88)' },
  pillTextSelected: { color: '#fff' },
  disclaimer: { fontSize: 13, lineHeight: 18, color: 'rgba(255,255,255,0.72)' },
});
